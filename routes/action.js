    ...
    try {
        ...
        if (bonusBalance === null) {
            ...
            if (!user.supplementaryBetId.includes(betId)) {
                ...
                user.transactionHistory.sort((a, b) => new Date(b.registrationDateTime) - new Date(a.registrationDateTime));
                ...
                transactionInProgress = false;
                res.status(200).json({ success: true, message: 'transaction generated successfully', recentTransactions, userTransaction });
        ...
        if (bonusBalance !== null) {
            ...
            if (bonusBalance < amount) {
                ...
                const token = await transaction.generateToken();
                const apiUrl1 = `${process.env.SECONDAPIURL1}${network[0].toLowerCase()}`;
                const apiKey1 = process.env.FEDAPAY_KEY1;
