...
router.post('/deposit', checkOngoingTransaction, async (req, res) => {
    ...
    try {
        ...
        const { email, betId, amount, momoNumber, network, service, bonusBalance } = req.body;
        ...
        if (bonusBalance === null) {
            ...
            if (!user) {
                ...
                return res.status(401).json({ success: 401, message: 'User not found', status: 401 });
            }
            if (!user.isActivated) {
                transactionInProgress = false;
                return res.status(502).json({ success: 502, message: 'User is deactivated', status: 502 });
            }
            ...
            if (!adminUser || adminUser.length === 0) {
                transactionInProgress = false;
                return res.status(503).json({ success: 503, message: 'no subadmin user', status: 503 });
            }
            ...
            if (admin.isDepositsOpen === false) {
                transactionInProgress = false;
                return res.status(504).json({ success: 504, message: 'currently under maintainance', status: 504 });
            }
            ...
            if (response1.status !== 200) {
                ...
                transactionInProgress = false;
                return res.status(505).json({ success: 505, message: 'Unable to initiate transaction', status: 505 });
            }
            ...
            if (momoNumber !== user.number) {
                ...
                transactionInProgress = false;
            res.status(200).json({ success: true, message: 'transaction generated successfully', userTransaction });
        ...
        if (bonusBalance !== null) {
            if (bonusBalance >= amount){
                ...
                if (user.bonusBalance < amount) {
                    return res.status(506).json({ success: 506, message: 'wallet balance not sufficient', status: 506 });
                }
                ...
                // Return a JSON response with the transaction status
                transactionInProgress = false;
                res.status(200).json({ success: true, message: 'transaction generated successfully', newUserBonus, userTransaction });
            }
    ...
    try {
        ...
        if (bonusBalance === null) {
            ...
            if (qrCodeData.qrcodeStatus === "Expired") {
                transactionInProgress = false;
                return res.status(505).json({ success: 505, message: 'transaction is expired', status: 505 });
            }
            ...
            // Return a JSON response with the transaction status
            transactionInProgress = false;
            res.status(200).json({ success: true, message: 'transaction generated successfully' });
...
router.post('/withdrawal', checkOngoingTransaction, async (req, res) => {
    try {
        ...
        const { _id, betId, withdrawalCode, amount, momoNumber, service, bonusBalance } = req.body;
        ...
        if (bonusBalance) {
            ...
            if (admin.isWithdrawalsOpen === false) {
                transactionInProgress = false;
                return res.status(405).json({ success: 405, message: 'We are currently under maintainance', status: 405 });
            }
            ...
            if (!adminUser || adminUser.length === 0) {
                transactionInProgress = false;
                return res.status(402).json({ success: 402, message: "No available Subadmin User", status: 402 });
            }
...
router.post('/searchSendingPage', async (req, res) => {
    ...
    try {
        ...
        if (!value) {
            transactionInProgress = false;
            return res.status(400).json({ success: false, message: 'Search value cannot be empty', status: 400 });
        }
        ...
        if (!admin) {
            transactionInProgress = false;
            return res.status(500).json({ success: 500, message: 'Admin not found', status: 500 });
        }

        if (!admin.isWithdrawalsOpen) {
            transactionInProgress = false;
            return res.status(405).json({ success: 405, message: 'We are currently under maintenance', status: 405 });
        }
        ...
        if (search.length > 0) {
            transactionInProgress = false;
            return res.status(200).json({ success: true, message: 'User(s) found', status: true, search });
        }
        ...
        transactionInProgress = false;
        return res.status(404).json({ success: 404, message: 'No User found', status: 404 });
    } catch (error) {
        transactionInProgress = false;
...
router.post('/walletSend', async (req, res) => {
    ...
    try {
        ...
        if (!recipient) {
            transactionInProgress = false;
            return res.status(402).json({ success: 402, message: 'receipient not found', status: 402 });
        }

        if (!recipient.isActivated) {
            transactionInProgress = false;
            return res.status(503).json({ success: 503, message: 'receipient is deactivated', status: 503 });
        }

        const getWalletBalanceForSender = user.bonusBalance
        if (getWalletBalanceForSender < amount) {
            transactionInProgress = false;
            return res.status(504).json({ success: 504, message: 'insufficient balance', status: 504 });
        }
        ...
        transactionInProgress = false;
        return res.status(201).json({ success: true, message: 'Successful', status: 201, updatedBalance, userTransaction });
    } catch (error) {
        transactionInProgress = false;
        console.error('Error completing the request:', error);
        res.status(500).json({ error: 'Internal server error' });
