...
router.post('/editProfileImage', checkOngoingTransaction, upload.single('image'), async (req, res) => {
    try {
        ...
        if (!file) {
            ...
            return res.send({ success: 400, message: 'No image uploaded', status: 400 });
        }

        const existingUser = await User.findOne({ email });
        if (!existingUser) {
            transactionInProgress = false
            return res.send({ success: 401, message: 'User not found', status: 401 });
        }

        if (!existingUser.isActivated) {
            transactionInProgress = false
            return res.send({ success: 502, message: 'User is deactivated', status: 502 });
        }
        ...
        if (existingUser.imageFileName) {
            try {
                await bucket.file(`profileImages/${existingUser.imageFileName}`).delete();
            } catch (err) {
                transactionInProgress = false
                return res.send({ success: 501, message: 'unable to delete previous file', status: 501 });
            }
        }
        ...
        blobStream.on('error', error => {
            ...
            return res.send({ success: 503, message: 'Failed to upload image', status: 503 });
        });

        blobStream.on('finish', async () => {
            await fileUpload.makePublic();
            const publicUrl = `https://storage.googleapis.com/${bucket.name}/profileImages/${encodeURIComponent(newFileName)}`;

            existingUser.image = publicUrl;
            existingUser.imageFileName = newFileName;
            await existingUser.save();
            transactionInProgress = false
            res.send({ success: true, message: "Profile image updated successfully", image: publicUrl });
        });
        transactionInProgress = false
...
router.post('/changeUserDetails', checkOngoingTransaction, signInValidate, async (req, res) => {
    try {
        transactionInProgress = true
        ...
        if (network.length < 1) {
            transactionInProgress = false
            return res.send({ success: 400, message: "invalid number", status: 400 });
        }
        ...
        if (user.isUser === true) {
            ...
            if (response.status !== 200) {
                transactionInProgress = false
                return res.status(402).send({ success: 402, message: 'failed to update information on fedapay', status: 402 });
            }
        }
