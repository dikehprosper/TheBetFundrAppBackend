...
const signInValidate = [
    ...
    check("password").isLength({ min: 4 }).withMessage("Password must be at least six characters")
]
...
router.post('/register', checkOngoingTransaction,  signInValidate , async (req, res) => {
    ...
    try {
        ...
        if (existingUser) {
            ...
            return res.status(400).send({ success: 400, message: 'User already exists', status: 400 });
        }
        ...
        try {
            ...
            transactionInProgress = false;
            return res.status(501).send({ success: 501, message: 'Failed to create Fedapay customer', status: 501 });
        }
        ...
        if (referrerId) {
            ...
            if (!user2) {
                transactionInProgress = false;
                return res.send({ success: 503, message: 'Referer does not exist', status: 503 });
            }
        ...
        transactionInProgress = false;
        res.header('auth-token', token).send({ message: "registered succesfully", token, success: true, savedUser: savedUser, status: 201 })
...
router.post('/login', checkOngoingTransaction, loginValidate,async (req, res) => {
    const errors = validationResult(req);
    ...
    try {
        ...
        if (!existingUser.pinState) {
            transactionInProgress = false;
            return res.status(504).send({ success: 504, message: 'Pin not set', status: 504, email: existingUser.email, });
        }
        ...
        const token = await jwt.sign(tokenData, tokenVlaue);
        transactionInProgress = false;
        res.header('auth-token', token).send({ success: true, message: "Logged in succesfully", token, status: 201, savedUser })
...
router.post('/setPin', checkOngoingTransaction, async (req, res) => {
    try {
        ...
        transactionInProgress = false;
        res.header('auth-token', token).send({ success: true, message: "set Pin succesfully", token, status: 201, savedUser })
    ...
    try {
        ...
        try {
            ...
            if (error.name === 'TokenExpiredError') {
                ...
                transactionInProgress = false;
                return res.status(504).send({ success: false, message: 'Token has expired', status: 504 });
            } else {
                transactionInProgress = false;
                // Throw other errors
                throw error;
        ...
        if (existingUser.pinState === false) {
            transactionInProgress = false;
            return res.status(501).send({ success: 503, message: 'User pin is not set', status: 503, email: existingUser.email, fullname: existingUser.fullname  });
        }
        ...
        transactionInProgress = false;
        res.header('auth-token', token).send({ success: true, message: "successfully fetched data", newToken, status: 201, savedUser })
...
router.post('/verifyUserPin', checkOngoingTransaction, loginValidate, async (req, res) => {
    ...
    try {
        ...
        if (!validPin) {
            transactionInProgress = false;
            return res.status(503).send({ success: 503, message: 'Invalid pin', status: 503 });
        }
